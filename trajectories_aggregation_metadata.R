...
# wps.in: id = lat_min, type = integer, title = Smallest latitude of spatial zone extent in degree. Range of values: -90Â° to 90Â°., value = "-90";
# wps.in: id = lat_max, type = integer, title = Biggest latitude of spatial zone extent in degree. Range of values: -90Â° to 90Â°., value = "90";
# wps.in: id = lon_min, type = integer, title =  Smallest longitude of spatial zone extent in degree. Range of values : -180Â° to 180Â°., value = "-180";
# wps.in: id = lon_max, type = integer, title =  Biggest longitude of spatial zone extent in degree. Range of values : -180Â° to 180Â°., value = "180";
...
# wps.in: id = spatial_reso, type = real, title = If spatial_grid=T, Spatial resolution that fits sides of the grid square polygons in degree. Range of values: 0.001Â° to 5Â°., value = "1";
...
######################## Packages
library(pacman)
p_load("rstudioapi","data.table","dplyr","lubridate","raster","rgeos","sp","rgdal","stringr","tictoc","rlang", "SDLfilter")
...
source("trajectories_aggregation.R")
source("metadata_generate.R")
...
if(object_type=='ais'){ csv_input_name <- paste0("ais_",gear,"_",year,"_compare_vms.csv")}
if(object_type=='vms'){ csv_input_name <- paste0("vms_",gear,"_",year,"_compare_ais.csv")}
...
file_path_input_data <- paste0("input/trajectories_aggregation/",csv_input_name)
public_link_input_data <- NA
institute_source <- "SFA"
...
file_path_metadata_model <- "input/trajectories_aggregation/metadata_input.csv"
...
## time column to choose has the format: %Y-%m-%d %H:%M:%S
# test<-input_dataset
# input_dataset$lat<-round(input_dataset$lat,3)
# input_dataset$lon<-round(input_dataset$lon,3)


if(object_type=='vms'){
#   input_dataset<-data.frame(cbind(input_dataset[,1:2],input_dataset[,6:8],input_dataset[,10],input_dataset[,15]))
# # colnames(test)<-c('id','id_traj','timestamp','lon','lat','speed','vessel')
# colnames(input_dataset)<-c('id','id_traj','timestamp','lon','lat','speed','speedcalc')
  ## VMS
  input_dataset<-input_dataset[,c('id','id_traj','timestamp','lat','lon','speed','id_object')]
  colnames(input_dataset)<-c('id','id_traj','timestamp','lat','lon','speed','vessel')
    
}
if(object_type=='ais'){
  # input_dataset<-data.frame(input_dataset[,c('id','id_traj','timestamp','lat','lon','speed','vessel')])
  input_dataset<-input_dataset[,c('id','id_traj','timestamp','lat','lon','speed','mmsi')]
  ## AIS
  # input_dataset<-input_dataset[,c('id','id_traj','timestamp','lat','lon','ais_speed','mmsi')]
  colnames(input_dataset)<-c('id','id_traj','timestamp','lat','lon','speed','mmsi')
...
identifier <- paste0("seychelles_",gear,"_",year,"_",object_type,if(aggregate_data==T){"_aggregated"},"_trajectories_",start_date,"_",final_date,"_",if(is.null(spatial_zone)){paste0(spatial_resolution_id,"deg","_")}else{label_spatial_zone},temporal_resolution_id,temporal_resolution_unit_id,"_",institute_source)
